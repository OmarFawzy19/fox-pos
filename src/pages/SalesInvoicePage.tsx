...
import { CustomersAPI, InventoryAPI, InvoicesAPI, Customer, InventoryItem, Invoice, InvoiceItem } from '@/lib/api';
...
export default function SalesInvoicePage() {
  ...
  const addItemToInvoice = () => {
    ...
    if (quantity > product.quantity) { toast.error(`الكمية المتاحة: ${product.quantity}`); return; }
    setItems(prev => [...prev, { productId: product.id, productName: product.name, quantity, unitPrice: product.salePrice, total: quantity * product.salePrice }]);
    setSelectedProduct(''); setQty('');
  };

  const handleSubmit = async () => {
    if (!customer || items.length === 0) { toast.error('يرجى اختيار العميل وإضافة منتجات'); return; }
    const paidAmount = Number(paid) || 0;
    const status = getPaymentStatus(paidAmount, total);
    const { number } = await InvoicesAPI.getNextNumber('sale');

    const invoice = await InvoicesAPI.add({
      number, type: 'sale', date: new Date().toISOString(),
      entityId: customer.id, entityName: customer.name,
      items, total, paid: paidAmount, remaining: total - paidAmount, status,
    });

    // Deduct stock
    await InventoryAPI.deductStock(items.map(i => ({ productId: i.productId, quantity: i.quantity })));

    // Update customer balance if remaining > 0
    if (total - paidAmount > 0) {
      await CustomersAPI.update(customer.id, { balance: customer.balance + (total - paidAmount) });
    }
          ...
          {customer && (
            <>
              <div className="border-t border-border pt-4">
                ...
                <div className="flex gap-2 mt-2">
                  <Select value={selectedProduct} onValueChange={setSelectedProduct}>
                    ...
                    <SelectContent>
                      {availableProducts.map(p => <SelectItem key={p.id} value={p.id}>{p.name} (متاح: {p.quantity}) - {formatCurrency(p.salePrice)}</SelectItem>)}
                    </SelectContent>
                  </Select>
              ...
              <div className="border-t border-border pt-4 space-y-3">
                <div className="flex justify-between text-base"><span>المجموع الفرعي</span><span>{formatCurrency(subtotal)}</span></div>
                <div>
                  <Label>نسبة الخصم (%)</Label>
                  <Input value={discount} onChange={e => { const v = Number(e.target.value); if (e.target.value === '' || (v >= 0 && v <= 100)) setDiscount(e.target.value); }} type="number" min="0" max="100" placeholder="0" />
                </div>
                {discountAmount > 0 && <div className="flex justify-between text-sm text-muted-foreground"><span>الخصم ({discountPercent}%)</span><span>- {formatCurrency(discountAmount)}</span></div>}
